...
from optimization import BertLAMB, BertNesLAMB
from apex.optimizers import FusedLAMB, FusedNesLAMB

from file_utils import PYTORCH_PRETRAINED_BERT_CACHE
from utils import is_main_process
from apex.parallel import DistributedDataParallel as DDP
from schedulers import LinearWarmUpScheduler, PolyWarmUpConstScheduler
...
def parse_arguments():
    ...
    parser.add_argument("--const_proportion",
                        default=0.01,
                        type=float,
                        help="Proportion of training to perform constant learning rate for. "
                             "E.g., 0.1 = 10%% of training.")
...
def setup_training(args):
    ...
    if args.local_rank == -1:
        ...
        #os.system('./scripts/ompi_bind_DGX1.sh')
...
def prepare_model_and_optimizer(args, device):
    ...
    if args.optimizer == 'lamb':
         ...
         optimizer = FusedNesLAMB(optimizer_grouped_parameters,
                                  lr=args.learning_rate)
    lr_scheduler = PolyWarmUpConstScheduler(optimizer, 
                                            warmup=args.warmup_proportion, 
                                            const=args.const_proportion,
                                            total_steps=args.max_steps)
    #optimizer = BertLAMB(optimizer_grouped_parameters,
    #                     lr=args.learning_rate,
    #                     warmup=args.warmup_proportion,
    #                     t_total=args.max_steps)
    #optimizer = BertNesLAMB(optimizer_grouped_parameters,
    #                        lr=args.learning_rate,
    #                        warmup=args.warmup_proportion,
    #                        const=args.const_proportion,
    #                        t_total=args.max_steps)
    ...
    return model, optimizer, checkpoint, global_step, lr_scheduler
...
def main():
    ...
    # Prepare optimizer
    model, optimizer, checkpoint, global_step, lr_scheduler = prepare_model_and_optimizer(args, device)
            ...
            for f_id in range(f_start_id + 1 , len(files)):
                ...
                for step, batch in enumerate(train_iter):
                    ...
                    if global_step >= args.max_steps or training_steps % (
                        ...
                        if is_main_process() or args.local_rank == 0:
