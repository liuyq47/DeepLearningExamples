...
def warmup_linear_const(x, warmup=0.002, const=0.001):
    if x < warmup:
        return x/warmup
    elif x < warmup + const:
        return 1.
    return max((x - 1. )/ (warmup + const - 1.), 0.)
...
SCHEDULES = {
    ...
    'warmup_linear_const':warmup_linear_const,
}
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                update = (next_m/bias_correction1) / ((next_v/bias_correction2).sqrt() + group['e'])
...
class BertNesLAMB(Optimizer):
    ...
    def __init__(self, params, lr=required, warmup=-1, const=-1, t_total=-1, schedule='warmup_linear_const',
                 b1=0.9, b2=0.999, e=1e-6, weight_decay=0.01):
        if lr is not required and lr < 0.0:
            raise ValueError("Invalid learning rate: {} - should be >= 0.0".format(lr))
        if schedule not in SCHEDULES:
        ...
        if not e >= 0.0:
            raise ValueError("Invalid epsilon value: {} - should be >= 0.0".format(e))
        defaults = dict(lr=lr, schedule=schedule, warmup=warmup, const=const, t_total=t_total,
                        b1=b1, b2=b2, e=e, weight_decay=weight_decay)
        super(BertNesLAMB, self).__init__(params, defaults)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                next_m.mul_(beta1).add_(1 - beta1, grad)
                next_v.mul_(beta2).addcmul_(1 - beta2, grad, grad)
                denom = (next_v/bias_correction2).sqrt() + group['e']
                update = (next_m/bias_correction1) / denom
                update_g = grad / denom
                ...
                if group['weight_decay'] > 0.0:
                    update += group['weight_decay'] * p.data
                    update_g += group['weight_decay'] * p.data

                update_norm = update.norm()
                update_g_norm = update_g.norm()

                if group['t_total'] != -1:
                    schedule_fct = SCHEDULES[group['schedule']]
                    lr_scheduled = group['lr'] * schedule_fct(state['step'] / group['t_total'], group['warmup'], group['const'])
                else:
                    lr_scheduled = group['lr']

                if update_norm != 0. and p_norm != 0.:
                    ratio_update = p_norm / update_norm
                else:
                    ratio_update = 1.
                if update_g_norm != 0. and p_norm != 0.:
                    ratio_g_update = p_norm / update_g_norm
                else:
                    ratio_g_update = 1.
                ratio_update *= beta1
                ratio_g_update *= 1. - beta1

                update_with_lr = (lr_scheduled * ratio_update) * update
                update_with_lr += (lr_scheduled * ratio_g_update) * update_g
                p.data.add_(-update_with_lr)

        if torch.distributed.get_rank() == 0:
            print("Step {} LR {}".format(state['step'], lr_scheduled))
